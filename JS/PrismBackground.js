...
(function () {
  ...
  // Idle-activity fade: wraps the whole effect layer (not the existing
  // per-triangle/per-gradient proximity opacity, which is untouched)
  // in an extra opacity layer that fades in on movement and fades
  // back out ~1.2s after the pointer stops - so the effect is
  // invisible at rest and only appears while the mouse/finger is
  // actually active. Call .poke() from each mode's onMove; CSS
  // transitions handle the actual easing, so retargeting mid-transition
  // (e.g. moving again during a fade-out) just smoothly redirects from
  ...
  function initGapGlow(doc) {
    ...
    // Thin wrapper purely for the idle-activity fade layer below - the
    // proximity-driven opacity toggle on glowRect itself is untouched.
    var fadeGroup = document.createElementNS(SVG_NS, 'g');
    fadeGroup.appendChild(glowRect);
    ...
    if (firstRect && firstRect.nextSibling) {
      svg.insertBefore(fadeGroup, firstRect.nextSibling);
    } else {
    ...
    var activityFade = createActivityFade(fadeGroup);
    var pendingX = null, pendingY = null, ticking = false, pointerInside = false;
